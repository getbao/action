...
// src/parse-tofu-outputs.ts
function parseTofuOutputs(json, log) {
  let parsed;
  try {
    parsed = JSON.parse(json);
  } catch (err) {
    throw new Error(
      `Failed to parse tofu output JSON: ${err.message}`
    );
  }
  const out = parsed;
  const dbId = out.d1_database_id?.value;
  const dbName = out.d1_database_name?.value;
  const kvId = out.kv_namespace_id?.value;
  ...
  if (missing.length > 0) {
    throw new Error(`Missing required tofu outputs: ${missing.join(", ")}`);
  }
  const result = {
    dbId: String(dbId),
  ...
  log(
    `Parsed tofu outputs: d1_database_id=${result.dbId}, d1_database_name=${result.dbName}, kv_namespace_id=${result.kvId}`
  );
  return result;
}

// entrypoints/parse-tofu-outputs.ts
void (async () => {
  await setupLogger();
  const logger2 = getLogger(["bao", "action", "parse-tofu-outputs"]);
  try {
    const chunks = [];
    ...
    const { dbId, dbName, kvId } = parseTofuOutputs(
      json,
      (msg) => logger2.info("{msg}", { msg })
    );
    setOutput("db_id", dbId);
    setOutput("db_name", dbName);
    setOutput("kv_id", kvId);
  } catch (err) {
    logger2.fatal("::error::{message}", { message: err.message });
    process.exit(1);
  }
})();
//# sourceMappingURL=parse-tofu-outputs.js.map
