...
// src/resolve-release.ts
var REQUIRED_VARS = ["BAO_API_KEY", "BAO_API_URL", "VERSION"];
async function resolveRelease(env) {
  const logger2 = getLogger(["bao", "action", "resolve-release"]);
  ...
  const hasRotationWorker = body.assets.some(
    (a) => a.name === "rotation-worker.js"
  );
  logger2.info("Resolved {version} \u2192 {resolved}, has_rotation_worker={has}", {
    version,
...
// entrypoints/resolve-release.ts
void (async () => {
  await setupLogger();
  const logger2 = getLogger(["bao", "action", "resolve-release"]);
  try {
    await resolveRelease(process.env);
  } catch (err) {
    logger2.fatal("::error::{message}", { message: err.message });
    process.exit(1);
  }
})();
//# sourceMappingURL=resolve-release.js.map
